  ...
  async function handleSuggestionMessages() {
    ...
    try {
      ...
      while (true) {
        ...
        accumulatedText = events.pop() || ''; // Keep the last incomplete event
        ...
        for (const event of events) {
          if (event.startsWith('event: chunk')) {
            ...
            const finalData = JSON.parse(event.split('data: ')[1]);
            // Split the accumulated text into separate questions
            const finalQuestions = finalData.questions[0].split('||').map((q: string) => q.trim());
            setSuggestions(finalQuestions);
            setStreaming(false);
          }
        }
      }
  ...
  return (
    <div className="container mx-auto my-8 p-6 bg-white rounded max-w-4xl">
      ...
      <div className="min-h-screen">
        ...
        <div className="mt-8">
          <Button onClick={handleSuggestionMessages} disabled={streaming}>
            {streaming ? 'Generating...' : 'Generate Questions'}
          </Button>
          <div className="mt-4">
            {suggestions.length > 0 && (
